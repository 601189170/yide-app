...
import static com.yide.scheduleview.ScheduleEventViewUtil.*;
...
public class ScheduleEventView extends View {
    ...
    private List<? extends ScheduleEvent> mPreviousPeriodEvents;
    private List<? extends ScheduleEvent> mCurrentPeriodEvents;
    private List<? extends ScheduleEvent> mNextPeriodEvents;
    ...
    private ScheduleEventViewLoader mWeekViewLoader;
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            switch (mCurrentScrollDirection) {
                ...
                case RIGHT:
                    ...
                    ViewCompat.postInvalidateOnAnimation(ScheduleEventView.this);
                    break;
                case VERTICAL:
    ...
    public ScheduleEventView(Context context, AttributeSet attrs, int defStyleAttr) {
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()) {
                    ...
                    if (left < right &&
                        ...
                        final String eventColor = mEventRects.get(i).event.getColor();
                        final String colorWithAlpha = getColorWithAlpha(eventColor, "CC");
                        mEventBackgroundPaint.setColor(Color.parseColor(colorWithAlpha) == 0 ? mDefaultEventColor : Color.parseColor(colorWithAlpha));
                        canvas.drawRoundRect(mEventRects.get(i).rectF, mEventCornerRadius, mEventCornerRadius, mEventBackgroundPaint);
                        //画一个矩形线为事件的头
                        mEventBackgroundPaint.setColor(Color.parseColor(eventColor) == 0 ? mDefaultEventColor : Color.parseColor(eventColor));
                        canvas.drawRoundRect(left, top, left + 5, bottom, mEventCornerRadius, mEventCornerRadius, mEventBackgroundPaint);
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && mEventRects.get(i).event.isAllDay()) {
                    ...
                    if (left < right &&
                        ...
                        mEventRects.get(i).rectF = new RectF(left, top, right, bottom);
                        mEventBackgroundPaint.setColor(Color.parseColor(mEventRects.get(i).event.getColor()) == 0 ? mDefaultEventColor : Color.parseColor(mEventRects.get(i).event.getColor()));
                        canvas.drawRoundRect(mEventRects.get(i).rectF, mEventCornerRadius, mEventCornerRadius, mEventBackgroundPaint);
                        drawEventTitle(mEventRects.get(i).event, mEventRects.get(i).rectF, canvas, top, left);
                    } else {
                        mEventRects.get(i).rectF = null;
    ...
    private class EventRect {
        ...
        public ScheduleEvent originalEvent;
        public RectF rectF;
        public float left;
        public float width;
        public float top;
         ...
         */
        public EventRect(ScheduleEvent event, ScheduleEvent originalEvent, RectF rectF) {
            this.event = event;
            this.rectF = rectF;
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mWeekViewLoader != null) {
            ...
            if (!isInEditMode() && (mFetchedPeriod < 0 || mFetchedPeriod != periodToFetch || mRefreshEvents)) {
                List<? extends ScheduleEvent> previousPeriodEvents = null;
                List<? extends ScheduleEvent> currentPeriodEvents = null;
                List<? extends ScheduleEvent> nextPeriodEvents = null;
     ...
     */
    private void cacheEvent(ScheduleEvent event) {
        if (event.getStartTime().compareTo(event.getEndTime()) >= 0) {
            return;
        }
        List<ScheduleEvent> splitedEvents = event.splitWeekViewEvents();
        for (ScheduleEvent splitedEvent : splitedEvents) {
     ...
     */
    private void sortAndCacheEvents(List<? extends ScheduleEvent> events) {
        sortEvents(events);
     ...
     */
    private void sortEvents(List<? extends ScheduleEvent> events) {
        Collections.sort(events, new Comparator<ScheduleEvent>() {
            @Override
            public int compare(ScheduleEvent event1, ScheduleEvent event2) {
     ...
     */
    public ScheduleEventViewLoader getWeekViewLoader() {
     ...
     */
    public void setWeekViewLoader(ScheduleEventViewLoader loader) {
     ...
     * </p>
     *
     * @param length Supported values are {@link ScheduleEventView#LENGTH_SHORT} and
     *               {@link ScheduleEventView#LENGTH_LONG}.
     */
    @Deprecated
    ...
    public interface EventClickListener {
         ...
         */
        void onEventClick(ScheduleEvent event, RectF eventRect);
    }
    ...
    public interface EventLongPressListener {
         ...
         * @param eventRect: view containing the clicked event.
         */
        void onEventLongPress(ScheduleEvent event, RectF eventRect);
    }
