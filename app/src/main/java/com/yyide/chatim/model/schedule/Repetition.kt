...
data class Repetition(
    ...
    var rule:MutableMap<String,String>? = null
) {
    companion object {
        fun getList(): List<Repetition> {
            ...
            val rule1 = mutableMapOf<String,String>()
            rule1["freq"] = "weekly"
            rule1["interval"] = "1"
            rule1["byday"] = "MO,TU,WE,TH,FR"
            remindList.add(Repetition("每个工作日（周一到周五）", false,rule1))
            //{"freq": "daily","interval": "1"}
            val rule2 = mutableMapOf<String,String>()
            rule2["freq"] = "daily"
            rule2["interval"] = "1"
            remindList.add(Repetition("每天", false,rule2))
            //{"freq": "weekly","interval": "1"}
            val rule3 = mutableMapOf<String,String>()
            rule3["freq"] = "weekly"
            rule3["interval"] = "1"
            remindList.add(Repetition("每周", false,rule3))
            //{"freq": "weekly","interval": "2"}
            val rule4 = mutableMapOf<String,String>()
            rule4["freq"] = "weekly"
            rule4["interval"] = "2"
            remindList.add(Repetition("每两周", false,rule4))
            //{"freq": "monthly","interval": "1"}
            val rule5 = mutableMapOf<String,String>()
            rule5["freq"] = "monthly"
            rule5["interval"] = "1"
            remindList.add(Repetition("每月", false,rule5))
            //{"freq": "yearly","interval": "1"}
            val rule6 = mutableMapOf<String,String>()
            rule6["freq"] = "yearly"
            rule6["interval"] = "1"
