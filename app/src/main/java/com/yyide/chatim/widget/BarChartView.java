...
public class BarChartView extends View {
    ...
    private int axisWidth = dip2px(getContext(), 0);//线的宽度
    private int axisColor = Color.LTGRAY;//线的颜色

    private int paddingLeft = dip2px(getContext(), 0);
    private int paddingTop = dip2px(getContext(), 0);
    private int paddingRight = dip2px(getContext(), 0);
    private int paddingBottom = dip2px(getContext(), 20);
    private int xTextHeight = dip2px(getContext(), 20);//X轴底部文字高度


    private TextPaint axisTextPaint;//画坐标轴的文字
    private int axisTextSize = dip2px(getContext(), 10);//文字大小
    private int axisTextColor = Color.BLACK;

    private TextPaint axisXTextPaint;//画坐标轴的文字
    private int axisXTextSize = dip2px(getContext(), 10);//文字大小
    private int axisXTextColor = Color.parseColor("#909399");

    private TextPaint barTextPaint;//画坐标轴的文字
    private int barTextSize = dip2px(getContext(), 10);//文字大小
    private int barTextColor = Color.parseColor("#909399");
    ...
    private int barWidth = dip2px(getContext(), 20);//柱子宽度
    private int barSpace = dip2px(getContext(), 30);//柱子间距
    ...
    private int barColor = getContext().getResources().getColor(R.color.colorAccent);//柱子颜色
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float agedata = maxdata / 6;//平均每份
        ageY = (height - paddingBottom - paddingTop) / 6;//线的每等分
        for (int i = 0; i < 6; i++) {
            if (i == 0) {
                ...
                canvas.drawText(i * agedata * 1 + "%", paddingLeft, y, axisTextPaint); //画Y轴刻度
            }
        ...
        for (int i = 0; i < yList.size(); i++) {
            ...
            canvas.drawText(ytext + "%", x0 + (barWidth - ytextwidth) / 2f, top0 - barTextHeight + 30, barTextPaint);
        }
