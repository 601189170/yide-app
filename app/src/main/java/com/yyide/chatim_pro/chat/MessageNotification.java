...
public class MessageNotification {
    ...
    private void createNotificationChannel(boolean isDialing) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (isDialing) {
                channel = new NotificationChannel(NOTIFICATION_CHANNEL_CALL, BaseApplication.getInstance().getString(R.string.call_notification), NotificationManager.IMPORTANCE_HIGH);
                channel.setSound(RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE), null);
                channel.setDescription(BaseApplication.getInstance().getString(R.string.background_tip));
                channel.setVibrationPattern(new long[]{0, 1000, 1000, 1000, 1000});
            } else {
                channel = new NotificationChannel(NOTIFICATION_CHANNEL_COMMON, BaseApplication.getInstance().getString(R.string.new_msg_notification), NotificationManager.IMPORTANCE_HIGH);
                channel.setDescription(BaseApplication.getInstance().getString(R.string.background_tip));
            }
    ...
    public void notify(V2TIMMessage msg) {
        ...
        String tickerStr = BaseApplication.getInstance().getString(R.string.new_msg);
        ...
        if (isDialing) {
            launch = new Intent(mContext, NewMainActivity.class);
